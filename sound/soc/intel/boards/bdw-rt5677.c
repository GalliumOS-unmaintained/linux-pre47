...
static struct snd_soc_jack bdw_headset;
/* Headset jack detection DAPM pins */
static struct snd_soc_jack_pin bdw_headset_pins[] = {
	{
		.pin = "Mic Jack",
...
static struct snd_soc_jack_gpio bdw_headset_gpios[] = {
    {
...
static int bdw_rt5677_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = snd_soc_card_jack_new(rtd->card, "Headset",
				    SND_JACK_HEADSET | SND_JACK_BTN_0, &bdw_headset,
				    bdw_headset_pins, ARRAY_SIZE(bdw_headset_pins));
	if (ret)
		return ret;

	ret = snd_soc_jack_add_gpios(&bdw_headset, ARRAY_SIZE(bdw_headset_gpios),
				     bdw_headset_gpios);
	if (!ret)
	    dev_err(codec->dev, "Can't add headset gpios\n");
